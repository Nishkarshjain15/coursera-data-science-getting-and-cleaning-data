...
The resulting tidy dataset contains 66 columns of averaged information (per subject and activity) corresponding to the mean and std of different meassurements for a total of 180 observations (rows). These observations include meassurements of magnitude and components in X,Y and Z direction for the linear acceleration, angular acceleration, linear jerk and angular jerk for both body and gravity components. Columns 1 and 2 correspond to the subject and activity. 
